...
SEASON = "2025"         # default season is the START year, e.g. "2025" = 2025/26
                          # override with --season on the command line
...
def main():
    ...
    parser.add_argument("--season", type=str, default=SEASON, help='Season start year, e.g. "2024" for 2024/25')
    ...
    # keep the default (current season) filename EXACTLY as before, since
    # build_merged_dataset.py and extract_understat_match_stats.py both
    # expect this exact name — only a non-default (historical) season
    # gets a suffixed filename, so nothing in the main pipeline breaks
    if SEASON == "2025":
        output_path = OUTPUT_DIR / "understat_player_match_stats.csv"
    else:
        output_path = OUTPUT_DIR / f"understat_player_match_stats_{SEASON}.csv"
