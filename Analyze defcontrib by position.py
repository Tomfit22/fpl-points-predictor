...
FULL_CANDIDATE_POOL = [
    ...
    # NEW: facing a high-possession opponent means more defensive actions
    # are needed — the most mechanically direct candidate of any of the
    # advanced-stats additions tested so far. Own team's pressing intensity
    ...
    # NEW: FPL's own official CBI/tackles/recoveries — literal sub-
    # components of defensive_contribution itself (verified: CBI + tackles
    # == defensive_contribution for DEF), sourced with zero legal risk
    # unlike the FBref-derived TklW/Int above
    "season_CBI", "roll5_CBI",
    "season_FPL_Tackles", "roll5_FPL_Tackles",
    "season_Recoveries", "roll5_Recoveries",
]
...
def get_feature_family(feature_name: str) -> str:
    ...
    # CBI + FPL_Tackles + Recoveries == def_contrib (a three-way SUM
    # relationship). Pairwise correlation pruning can't catch this, since
    # no single pair necessarily exceeds the threshold even though the
    # combination is perfectly collinear — confirmed on real data (NaN
    # p-values persisted even after the correlation-based fix). Grouping
    # them into one family is the actual fix: only one of these four
    # ever gets selected, eliminating the multicollinearity at its root
    # instead of trying to catch it after the fact.
    if name in ("def_contrib", "CBI", "FPL_Tackles", "Recoveries"):
        name = "def_contrib_component"
...
def select_features_for_position(pos_df: pd.DataFrame, n_events: int, target_col: str) -> list:
    ...
    candidates = best_per_family.sort_values("abs_corr", ascending=False)["feature"].tolist()

    # proactive pruning of near-duplicate features (correlation > 0.98) —
    # family-dedup alone isn't enough now that CBI/Tackles/Recoveries are
    # DIFFERENT families from def_contrib but still near-perfectly
    # collinear with it (they're literal sub-components), which broke
    # logistic regression convergence in practice on real data
    selected = []
    for f in candidates:
        if len(selected) >= n_feats:
            break
        too_similar = any(abs(pos_df[f].corr(pos_df[g])) > 0.98 for g in selected)
        if not too_similar:
            selected.append(f)
...
def logistic_regression(df: pd.DataFrame, features: list):
    ...
    try:
        ...
        print(f"  (standard MLE failed ({e}) — falling back to regularized fit. This happens "
              f"when CBI/Tackles/Recoveries are included alongside def_contrib, since they're "
              f"literal sub-components of it — expected multicollinearity, not a data error.)")
        model = sm.Logit(y, X).fit_regularized(disp=0, alpha=0.1)
    if "const" not in model.params.index:
        print("  (WARNING: 'const' missing from fitted params — design matrix likely rank-deficient.)")
        coefs = model.params
    else:
    ...
    result = pd.DataFrame({
        ...
        "p_value": model.pvalues.reindex(coefs.index),
    }).sort_values("p_value")
    return model, result
...
def analyze_position(df: pd.DataFrame, position: str):
    ...
    if len(pos_df) == 0:
        print(f"\nNo rows found for position {position} — skipping.")
        return
    ...
    print(f"\n--- OUT-OF-SAMPLE VALIDATION (this is the real test of whether the new possession/PPDA features actually help) ---")
    evaluate_out_of_sample(pos_df, features)


def evaluate_out_of_sample(pos_df: pd.DataFrame, features: list):
    """Same lesson learned from bonus points (leaky baseline bug) and clean
    sheets (calibration testing) — baseline must be derived from TRAINING
    data only, never from the test set's own true rate."""
    cutoff_gw = pos_df["gameweek"].quantile(0.75)
    train = pos_df[pos_df["gameweek"] <= cutoff_gw]
    test = pos_df[pos_df["gameweek"] > cutoff_gw]

    if train["hit_dc_threshold"].sum() < 20 or len(test) == 0:
        print("  Not enough training events for a meaningful held-out test — skipping.")
        return

    X_train = sm.add_constant(train[features].fillna(0))
    try:
        model = sm.Logit(train["hit_dc_threshold"], X_train).fit(disp=0)
        if not model.mle_retvals.get("converged", True):
            raise np.linalg.LinAlgError("did not converge")
    except np.linalg.LinAlgError:
        model = sm.Logit(train["hit_dc_threshold"], X_train).fit_regularized(disp=0, alpha=0.1)

    X_test = sm.add_constant(test[features].fillna(0), has_constant="add")
    try:
        pred_proba = model.predict(X_test)
    except ValueError as e:
        print(f"  Could not generate held-out predictions ({e}) — likely severe collinearity "
              f"caused the fallback fit to drop columns unpredictably. Skipping this position's "
              f"out-of-sample check; the in-sample SHAP/logistic results above are still valid.")
        return

    model_brier = float(np.mean((pred_proba - test["hit_dc_threshold"]) ** 2))
    train_rate = train["hit_dc_threshold"].mean()  # non-leaky: TRAINING rate, not test's own true rate
    naive_brier = float(np.mean((train_rate - test["hit_dc_threshold"]) ** 2))

    print(f"  Train: {len(train)} rows (gw <= {cutoff_gw:.0f}) | Test: {len(test)} rows (gw > {cutoff_gw:.0f})")
    print(f"  Naive baseline (training-set hit rate, {train_rate:.1%}): Brier = {naive_brier:.4f}")
    print(f"  Model Brier = {model_brier:.4f}")
    if model_brier >= naive_brier:
        print("  *** Does NOT beat the naive baseline. ***")
    else:
        print(f"  Beats naive baseline by {naive_brier - model_brier:.4f} Brier points.")
