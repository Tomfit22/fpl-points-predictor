...
CANDIDATE_POOL = [
    ...
    "was_home_int",
    # NEW: possession and PPDA (pressing intensity) are direct proxies for
    # territorial/defensive control — plausible genuine improvement over
    # goals/shots/xG alone, being tested here rather than assumed
    "own_season_possession", "own_roll5_possession",
    "opp_season_possession", "opp_roll5_possession",
    "own_season_ppda", "own_roll5_ppda",
    "opp_season_ppda", "opp_roll5_ppda",
]
...
def poisson_model(train: pd.DataFrame, features: list):
    ...
    if "const" not in model.params.index:
        # can happen with a rank-deficient design matrix (too many
        # collinear features relative to rows) — statsmodels silently
        # drops columns rather than erroring, which then breaks a plain
        # .drop("const") downstream
        print("  (WARNING: 'const' missing from fitted params — design matrix "
              "may be rank-deficient. Check for highly correlated features "
              "or too few rows relative to feature count.)")
        coefs = model.params
    else:
